...
function getCacheKey(promptParts) {
  ...
  for (const p of promptParts) {
    ...
    if (p._originalPath) {
      try {
         hash.update(fs.readFileSync(p._originalPath))
      } catch (e) {
         hash.update(p._originalPath)
      }
    } else if (p.inlineData) {
      hash.update(p.inlineData.data)
    }
...
export async function callWithRetry(promptParts, modelIdx = 0, attempt = 0) {
  ...
  // Clean up _originalPath before sending to Gemini API
  for (const p of promptParts) {
    if (p._originalPath) delete p._originalPath
  }
...
const SYNONYMS = {
  ...
  color: {
    blue:   ['navy', 'navy blue', 'dark blue', 'light blue', 'sky blue', 'royal blue', 'cobalt', 'indigo', 'teal blue', 'turquoise'],
  ...
  fit: {
    loose:   ['flare', 'flared', 'a-line', 'free flowing', 'breezy', 'oversized', 'comfort'],
    regular: ['standard', 'normal', 'classic', 'straight', 'relaxed'],
...
function fuzzyMatch(val1, val2, category) {
  ...
  if (['other', 'not determinable'].includes(v1) || ['other', 'not determinable'].includes(v2)) {
    return 'skip'
  }
...
export function compareAttributesDeterministic(anchorAttrs, catalogAttrs, declaredAttrs) {
  return ATTR_CONFIG.map(({ key, severity, strict }) => {
    ...
    if (key === 'overall_length' && anchorAttrs.cv_overall_length) {
      ...
      const isKurti = declaredAttrs?.garment_type === 'Kurti' || declaredAttrs?.garment_type === 'Dress'
      if (isKurti && llmCvMatch === 'mismatch' && anchorVal !== 'Not determinable') {
...
- DO NOT guess. If something is not visible or determinable, use "Not determinable".`
  ...
  // [DISABLED] Groq decommissioned their vision models
  /*
  try {
...
/**
 * Extract catalog attributes using Gemini API with CATALOG_PROMPT_JOINT
 */
export async function extractCatalogAttributes(catalogPaths, anchorPaths = []) {
  ...
  console.log(`[EXTRACT-CATALOG] Running extraction on ${catalogPaths.length} catalog images...`)
  let geminiAttrs = null
  
  if (apiKeys.length > 0) {
    try {
      console.log('[EXTRACT-CATALOG] Phase 2: Gemini Vision (Catalog + Anchor joint extraction)...')
      const inlineImages = await Promise.all([
        ...anchorPaths.slice(0, 2).map(p => fileToInlineData(p)),
        ...catalogPaths.slice(0, 3).map(p => fileToInlineData(p))
      ])
      const promptParts = [
        ...inlineImages,
        { text: CATALOG_PROMPT_JOINT }
      ]
      const rawText = await callWithRetry(promptParts)
      geminiAttrs = parseJSON(rawText)
      console.log(`[EXTRACT-CATALOG] Gemini Vision extracted ${Object.keys(geminiAttrs).length} attributes:`, JSON.stringify(geminiAttrs))
    } catch (err) {
      console.warn('[EXTRACT-CATALOG] Gemini Vision failed:', err.message)
    }
  }
  ...
  if (geminiAttrs) {
    for (const [key, value] of Object.entries(geminiAttrs)) {
      if (value && value.value && value.value !== 'None' && value.value !== 'Not determinable') {
        attrs[key] = {
          value: value.value,
          confidence: 'HIGH',
          source: 'Gemini-Vision'
    ...
    // FALLBACK
    console.warn('[EXTRACT-CATALOG] Falling back to Local AI')
    const [vitResult, clipZsResult] = await Promise.all([
      runVitInference(primaryPath).catch(() => null),
      runClipZeroShot(primaryPath).catch(() => null)
    ])
    ...
    if (vitResult) {
      ...
      for (const [vitKey, canonicalKey] of Object.entries(VIT_TO_CANONICAL)) {
        if (vitResult[vitKey]) {
          attrs[canonicalKey] = { value: vitResult[vitKey].value, confidence: normalizeConfidence(vitResult[vitKey].confidence), source: 'ViT' }
        }
      }
    }
    ...
    if (clipZsResult) {
      for (const [key, val] of Object.entries(clipZsResult)) {
        if (!attrs[key] && val.value) {
          attrs[key] = { value: val.value, confidence: normalizeConfidence(val.confidence), source: 'CLIP-ZeroShot' }
        }
      }
    }
  }

  const cvRatio = await getGarmentBoundingBoxRatio(primaryPath).catch(() => null)
  if (cvRatio && attrs.overall_length) {
    attrs.cv_overall_length = {
      value: cvRatio.length_category,
      confidence: 'HIGH',
    ...
    if (cvRatio.length_category !== attrs.overall_length.value) {
      attrs.overall_length.cross_check = `CV Segmentation says "${cvRatio.length_category}"`
    }
  }
